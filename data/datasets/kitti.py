...
class KITTIDataset(Dataset):
	...
	def get_calibration(self, idx, use_right_cam=False):
		...
		base_name = file_name[:4] + ".txt"
		calib_filename = os.path.join(self.calib_dir, self.label_files[idx])
		base_filename = os.path.join(self.calib_dir, base_name)

		#if not os.path.exists(calib_filename):
			#f = open(base_filename, "r")
			#copy = open(calib_filename, "wt")
			#line = f.read()
			#copy.write(line)
			#f.close()
			#copy.close()
			#print("file does not exist")
			#shutil.copyfile(base_filename, calib_filename)
		#calib_filename = self.calib_dir
